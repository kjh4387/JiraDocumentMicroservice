...
class ExpertRepository(Repository[Expert]):
    ...
    def __init__(self, db_connection: DatabaseConnection, logger: Optional[logging.Logger] = None):
        ...
        self.logger.debug("ExpertRepository initialized")
    
    def find_by_id(self, id: str) -> Optional[Expert]:
        """ID로 전문가 조회"""
        self.logger.debug("Finding expert by ID", id=id)
        ...
        try:
            ...
            if not result:
                self.logger.warning("Expert not found", id=id)
                return None
            
            expert = Expert(**result[0])
            self.logger.debug("Expert found", id=id, name=expert.name)
            return expert
        except Exception as e:
            self.logger.error("Database error while finding expert", id=id, error=str(e))
            raise DatabaseError(f"Failed to find expert with ID {id}: {str(e)}")
    
    def find_by_criteria(self, criteria: Dict[str, Any]) -> List[Expert]:
        """조건에 맞는 전문가 목록 조회"""
        self.logger.debug("Finding experts by criteria", criteria=criteria)
        conditions = []
        params = []
        ...
        try:
            ...
            self.logger.debug("Experts found", count=len(experts))
            return experts
        except Exception as e:
            self.logger.error("Database error while finding experts by criteria", criteria=criteria, error=str(e))
            raise DatabaseError(f"Failed to find experts by criteria: {str(e)}")
    
    def find_by_name(self, name: str) -> List[Expert]:
        """이름으로 전문가 조회"""
        self.logger.debug("Finding experts by name", name=name)
        query = "SELECT * FROM experts WHERE name LIKE %s"
        
        try:
            result = self.db.execute_query(query, (f"%{name}%",))
            experts = [Expert(**row) for row in result]
            self.logger.debug("Experts found", count=len(experts), name=name)
            return experts
        except Exception as e:
            self.logger.error("Database error while finding experts by name", name=name, error=str(e))
            raise DatabaseError(f"Failed to find experts by name {name}: {str(e)}")
    
    def save(self, expert: Expert) -> Expert:
        """전문가 정보 저장"""
        self.logger.debug("Saving expert", id=expert.id, name=expert.name)
        ...
        if existing:
            ...
            try:
                ...
                self.logger.info("Expert updated", id=expert.id, name=expert.name)
            except Exception as e:
                self.logger.error("Database error while updating expert", id=expert.id, error=str(e))
            ...
            try:
                self.db.execute_query(query, params)
                self.logger.info("Expert created", id=expert.id, name=expert.name)
            except Exception as e:
                self.logger.error("Database error while creating expert", id=expert.id, error=str(e))
    ...
    def delete(self, id: str) -> bool:
        """전문가 정보 삭제"""
        self.logger.debug("Deleting expert", id=id)
        
        # 기존 전문가 확인
        existing = self.find_by_id(id)
        
        if not existing:
            self.logger.warning("Cannot delete: Expert not found", id=id)
            return False
        
        query = "DELETE FROM experts WHERE id = %s"
        try:
            self.db.execute_query(query, (id,))
            self.logger.info("Expert deleted", id=id)
            return True
        except Exception as e:
            self.logger.error("Database error while deleting expert", id=id, error=str(e))
